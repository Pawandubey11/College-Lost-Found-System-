...
const audit_logs: any[] = [
  { id: 1, user_id: 1, action: 'DATABASE_SEED', target_type: 'SYSTEM', target_id: 0, details: 'Self-contained Vercel serverless database initialized.', created_at: '2026-09-05 00:00:00' }
];
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'HEALTHY',
    service: 'Smart College Lost & Found API (Vercel Unified Serverless)',
...
app.get('/api/items/my-reports', authenticateToken, (req: any, res: any) => {
...
app.post('/api/items/report', authenticateToken, (req: any, res: any) => {
  ...
  if (cat) { (newItem as any).category_name = cat.name; (newItem as any).category_icon = cat.icon_name; }
  if (loc) { (newItem as any).campus_zone = loc.campus_zone; (newItem as any).building_name = loc.building_name; (newItem as any).floor_level = loc.floor_level; }
  if (rep) { (newItem as any).reporter_name = rep.full_name; (newItem as any).reporter_role = rep.role; (newItem as any).reporter_department = rep.department; }

  items.unshift(newItem);
  res.status(201).json({ message: 'Report created successfully.', itemId: newItem.id, matchesDetected: 0 });
});

app.put('/api/items/:id/status', authenticateToken, (req: any, res: any) => {
  const item = items.find(i => i.id === Number(req.params.id));
  if (!item) return res.status(404).json({ error: 'Item not found.' });
...
// Claims Routes
app.post('/api/claims/item/:itemId', authenticateToken, (req: any, res: any) => {
...
app.get('/api/claims/my-claims', authenticateToken, (req: any, res: any) => {
  const myClaims = claims.filter(c => c.claimant_id === req.user.id);
  res.json({ claims: myClaims });
});

app.get('/api/claims/received', authenticateToken, (req: any, res: any) => {
...
app.put('/api/claims/:claimId/decision', authenticateToken, (req: any, res: any) => {
...
app.put('/api/admin/users/:userId/role', authenticateToken, (req: any, res: any) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Admin access required.' });
...
app.get('/api/admin/reports', authenticateToken, (req: any, res: any) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Admin access required.' });
  res.json({ reports: items });
});

app.get('/api/admin/claims', authenticateToken, (req: any, res: any) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Admin access required.' });
  res.json({ claims });
});

app.get('/api/admin/audit', authenticateToken, (req: any, res: any) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Admin access required.' });
